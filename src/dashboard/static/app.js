...
const App = {
  ...
  async showOnboardingStep() {
    ...
    if (step === 0) {
      ...
      // Detect providers
      el.innerHTML =
        '<h2 class="onboarding-title">Detecting Providers...</h2>' +
      ...
      try {
        ...
        if (data.localServers && data.localServers.length > 0) {
          ...
          for (var i = 0; i < data.localServers.length; i++) {
            ...
            html +=
              '<button class="onboarding-provider-btn" onclick="App.selectOnboardingProvider(&#39;ollama&#39;, &#39;' +
              App.escapeHtml(s.models[0] || '') +
              '&#39;, &#39;' +
              App.escapeHtml(s.url) +
              '&#39;)">' +
              '<strong>' +
              App.escapeHtml(s.name) +
              '</strong> <span class="onboarding-model">' +
              App.escapeHtml(s.models.slice(0, 3).join(', ')) +
              '</span>' +
              '</button>';
          }
        }
        ...
        if (data.envProviders && data.envProviders.length > 0) {
          ...
          for (var j = 0; j < data.envProviders.length; j++) {
            ...
            html +=
              '<button class="onboarding-provider-btn" onclick="App.selectOnboardingProvider(&#39;' +
              App.escapeHtml(p) +
              '&#39;)">' +
              '<strong>' +
              App.escapeHtml(p.charAt(0).toUpperCase() + p.slice(1)) +
              '</strong>' +
              '</button>';
          }
        }
        ...
        html +=
          '<div class="onboarding-free-tier" style="margin-top:20px;border-top:1px solid rgba(255,255,255,0.1);padding-top:20px;">';
        html +=
          '<button class="onboarding-provider-btn" style="background:linear-gradient(135deg,#2F6BFF,#00D2FF);border:none;width:100%;padding:14px;border-radius:10px;color:white;font-size:1.1em;font-weight:600;cursor:pointer;margin-bottom:12px;" onclick="App.selectFreeProxy()">';
        html +=
          '<strong>Try Free</strong> <span class="onboarding-model" style="opacity:0.9;">No API key needed — powered by Claude Sonnet</span>';
        html += '</button>';
        html +=
          '<p style="text-align:center;font-size:0.8em;opacity:0.5;">Free tier: 20 messages/day. Add your own key for unlimited use.</p>';
        html += '</div>';
        html +=
          '<div class="onboarding-manual-key" style="margin-top:16px;border-top:1px solid rgba(255,255,255,0.1);padding-top:16px;">';
        if (
          (!data.localServers || data.localServers.length === 0) &&
          (!data.envProviders || data.envProviders.length === 0)
        ) {
        ...
        if (
          ...
          html +=
            '<button class="btn-continue" onclick="App.nextOnboardingStep()" style="margin-top:12px;opacity:0.5;font-size:0.85em;">Skip for Now</button>';
        }
        html += '</div>';
        ...
        el.innerHTML =
          '<h2 class="onboarding-title">Provider Detection</h2>' +
  ...
  async selectFreeProxy() {
    try {
      ...
      if (!res.ok) {
        console.error('selectFreeProxy failed: HTTP ' + res.status);
      }
    } catch (err) {
  ...
  async selectOnboardingProvider(provider, model, baseUrl) {
    try {
      ...
      if (!res.ok) {
        console.error('selectOnboardingProvider failed: HTTP ' + res.status);
      }
    } catch (err) {
  ...
  showPanel(name) {
    document
      ...
      .forEach((pill) => pill.classList.toggle('active', pill.dataset.panel === name));
    document
  ...
  addImageFromFile(file) {
    ...
    reader.onload = (e) => {
      ...
      thumb.innerHTML =
        ...
        dataUrl +
        '" alt="Pasted image" />' +
        '<button class="remove-image" data-idx="' +
        idx +
        '">&times;</button>';
    ...
    if (images && images.length > 0) {
      ...
      for (let i = 0; i < images.length; i++) {
        imagesHtml +=
          ...
          ';base64,' +
          images[i].data +
          '" class="chat-msg-image" alt="Attached image" />';
      }
      imagesHtml += '</div>';
    }
    let contentHtml;
  ...
  async streamChat(message, images) {
    ...
    contentEl.innerHTML =
      '<div class="chat-thinking"><span class="thinking-dot"></span><span class="thinking-dot"></span><span class="thinking-dot"></span></div>';
    let fullText = '';
    let totalTokens = 0;
    let hasError = false;
    try {
      ...
      if (!hasError && err.name !== 'AbortError') {
        contentEl.innerHTML =
          ...
          '<div class="chat-error-icon">&#9888;</div>' +
          '<div class="chat-error-text">' +
          App.escapeHtml(App.friendlyError(String(err))) +
          '</div>' +
          '<button class="chat-retry-btn" onclick="App.retryLastMessage()">Retry</button>' +
          '</div>';
      }
  ...
  friendlyError(msg) {
    ...
    if (m.includes('401') || m.includes('unauthorized') || m.includes('authentication'))
      return 'API key is invalid or expired. Check Settings to update your key.';
    if (m.includes('402') || m.includes('insufficient') || m.includes('credit') || m.includes('billing'))
      return 'API credits exhausted. Add credits to your API account to continue.';
    if (m.includes('timeout') || m.includes('timed out'))
      return 'Request timed out. Try breaking the task into smaller steps.';
    if (m.includes('network') || m.includes('fetch') || m.includes('econnrefused'))
      return 'Network error — cannot reach the AI provider. Check your internet connection.';
    if (m.includes('overloaded') || m.includes('503') || m.includes('529'))
      return 'AI provider is overloaded. Wait a moment and try again.';
    return msg;
  },
  ...
  sendCapabilityDemo(category) {
    var demos = {
      ...
      automate:
        'Show me your automation capabilities. Check the system health, list recent git commits, and give me a status report.',
      create:
        'Write a professional LinkedIn post announcing an AI-powered coding agent that can autonomously write, test, and deploy code.',
      connect:
        'Show me all the app integrations you support and how they work. List your connectors and their capabilities.',
  ...
  async browseDir(dirPath) {
    ...
    try {
      ...
      if (breadcrumb) {
        ...
        for (var i = 0; i < parts.length; i++) {
          ...
          if (isLast) {
            ...
            crumbs +=
              '<span class="breadcrumb-item" onclick="App.browseDir(&#39;' +
              this.escapeHtml(built) +
              '&#39;)">>' +
              this.escapeHtml(parts[i]) +
              '</span>';
          }
        }
      ...
      if (data.parent) {
        html +=
          '<div class="file-item directory" onclick="App.browseDir(&#39;' +
          this.escapeHtml(data.parent) +
      ...
      for (var j = 0; j < data.items.length; j++) {
        ...
        if (item.type === 'directory') {
          html +=
            ...
            '&#39;)"><span class="file-icon">&#128193;</span> <span class="file-name">' +
            this.escapeHtml(item.name) +
            '</span></div>';
        } else {
          var sizeStr = item.size ? this.formatBytes(item.size) : '';
          html +=
            '<div class="file-item file" onclick="App.previewFile(&#39;' +
            this.escapeHtml(item.path) +
            '&#39;, &#39;' +
            this.escapeHtml(item.name) +
            '&#39;)"><span class="file-icon">' +
            this.getFileIcon(item.ext) +
  ...
  async loadStatus() {
    try {
      ...
      grid.innerHTML =
        ...
        memTotal +
        ' MB</div>' +
        '<div class="status-card-bar"><div class="status-bar-fill" style="width:' +
        Math.round((data.processMemory.heapUsed / data.processMemory.heapTotal) * 100) +
        '%"></div></div>' +
        '</div>' +
        '<div class="status-card">' +
        ...
        '<div class="status-card-value">' +
        this.escapeHtml(data.provider) +
        '</div>' +
        '<div class="status-card-sub">' +
        this.escapeHtml(data.model) +
        '</div>' +
        '</div>' +
        '<div class="status-card">' +
        ...
        '<div class="status-card-value">' +
        this.escapeHtml(data.platform + '-' + data.arch) +
        '</div>' +
        '<div class="status-card-sub">Node ' +
        this.escapeHtml(data.nodeVersion) +
        ' &middot; ' +
        data.cpus +
        ' cores</div>' +
        '</div>' +
        ...
        '<div class="status-card-value">v' +
        this.escapeHtml(data.version) +
        '</div>' +
        '<div class="status-card-sub">PID: ' +
        data.pid +
        '</div>' +
        '</div>';
    } catch (err) {
  ...
  saveConversations() {
    ...
    this._saveConvTimer = setTimeout(() => {
      this._saveConversationsNow();
    }, 1000);
  },
  ...
  getActiveConversation() {
    ...
    return this.conversations.find(function (c) {
      return c.id === App.activeConversationId;
    });
  },
  ...
  deleteConversation(id, evt) {
    ...
    this.conversations = this.conversations.filter(function (c) {
      return c.id !== id;
    });
    ...
    list.innerHTML = this.conversations
      ...
      .map(function (conv) {
        ...
        return (
          ...
          (isActive ? ' active' : '') +
          '" onclick="App.switchConversation(&#39;' +
          conv.id +
          '&#39;)">' +
          '<div class="chat-sidebar-item-title">' +
          App.escapeHtml(conv.title) +
          '</div>' +
          '<div class="chat-sidebar-item-meta">' +
          msgCount +
          ' msgs &middot; ' +
          App.escapeHtml(timeAgo) +
          '</div>' +
          '<button class="chat-sidebar-item-delete" onclick="App.deleteConversation(&#39;' +
          conv.id +
          '&#39;, event)" title="Delete">&times;</button>' +
          '</div>'
        );
      })
      .join('');
  ...
  async loadSettingsMeta() {
    ...
    try {
      ...
      if (backendEl) backendEl.textContent = 'Healthy';
    } catch (err) {
      if (backendEl) backendEl.textContent = 'Unavailable';
    }

    try {
      var toolData = await this.fetch('/api/command/tools');
      if (toolsEl) toolsEl.textContent = (toolData.tools ? toolData.tools.length : 0) + ' available';
    } catch (err) {
      if (toolsEl) toolsEl.textContent = 'Unavailable';
    }

    if (window.electronAPI && window.electronAPI.getBackendStatus && backendEl) {
      try {
  ...
  async saveSettings() {
    ...
    try {
      ...
      if (apiKey) {
        ...
        document.getElementById('settings-api-key').placeholder = 'Key configured (hidden)';
      }
    } catch (err) {
  ...
  updateAgentStatus(status, data) {
    ...
    if (status === 'working') {
      ...
      indicator.innerHTML =
        '<span class="agent-status-dot working"></span><span class="agent-status-text">' +
        this.escapeHtml(label) +
        '</span><span class="agent-status-timer" id="agent-timer">0s</span>';
      ...
      indicator.innerHTML =
        '<span class="agent-status-dot queued"></span><span class="agent-status-text">Queued (' +
        (data?.queueLength || 1) +
        ' pending)</span>';
    } else {
    ...
    try {
      ...
      list.innerHTML = data.notifications
        ...
        .map(function (n) {
          ...
          var priorityClass =
            n.priority === 'high' ? ' notification-high' : n.priority === 'low' ? ' notification-low' : '';
          var readClass = n.read ? ' notification-read' : '';
          var timeAgo = App.relativeTime(n.createdAt);
          var actionHtml = n.action
            ? ' <button class="notification-action-btn" onclick="event.stopPropagation();App.runNotificationAction(&#39;' +
              App.escapeHtml(n.action.command) +
              '&#39;)">' +
              App.escapeHtml(n.action.label) +
              '</button>'
            : '';
          return (
            ...
            priorityClass +
            readClass +
            '" onclick="App.dismissNotification(&#39;' +
            n.id +
            '&#39;)">' +
            '<div class="notification-card-title">' +
            App.escapeHtml(n.title) +
            '</div>' +
            '<div class="notification-card-msg">' +
            App.escapeHtml(n.message).substring(0, 120) +
            actionHtml +
            '</div>' +
  ...
  async loadProfile() {
    ...
    try {
      ...
      var html = fields
        .map(function (f) {
          ...
          if (f.type === 'select') {
            ...
            input =
              '<input type="text" class="tool-input profile-input" data-key="' +
              f.key +
              '" value="' +
              App.escapeHtml(f.value) +
              '" placeholder="' +
              App.escapeHtml(f.placeholder || '') +
              '" />';
          }
          return (
            '<div class="profile-field"><label class="tool-label">' +
      ...
      // Show stats
      var actions = Object.entries(p.commonActions || {})
        .sort(function (a, b) {
          return b[1] - a[1];
        })
        .slice(0, 5);
      if (actions.length > 0) {
        html +=
          '<div class="profile-stats"><span class="memory-heading" style="margin-top:12px;display:block">Activity</span>';
        html += actions
          .map(function (a) {
      ...
      if (p.connectedServices && p.connectedServices.length > 0) {
        html +=
          '<div class="profile-stats"><span class="memory-heading" style="margin-top:8px;display:block">Connected Services</span>';
        html += p.connectedServices
          .map(function (s) {
  ...
  async loadMemoryFiles() {
    ...
    try {
      ...
      if (!data.files || data.files.length === 0) {
        container.innerHTML =
          '<p class="workflow-no-fields">No memory files yet. Memory is created automatically as you use CodeBot.</p>';
        return;
      }
      ...
      container.innerHTML = data.files
        .map(function (f) {
          ...
          return (
            '<div class="memory-file-card" onclick="App.openMemoryEditor(&#39;' +
            App.escapeHtml(f.scope) +
            '&#39;, &#39;' +
            App.escapeHtml(f.file) +
            '&#39;)">' +
  ...
  async saveMemoryFile() {
    ...
    try {
      await apiFetch(
        '/api/memory/' +
          encodeURIComponent(this.currentMemoryFile.scope) +
          '/' +
          encodeURIComponent(this.currentMemoryFile.file),
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  async loadWorkflows() {
    ...
    try {
      ...
      var catHtml =
        '<button class="category-chip active" data-cat="all" onclick="App.filterWorkflows(&#39;all&#39;)">All</button>';
      var seenCats = {};
      for (var i = 0; i < data.workflows.length; i++) {
        ...
        if (!seenCats[cat] && data.categories[cat]) {
          seenCats[cat] = true;
          catHtml +=
            '<button class="category-chip" data-cat="' +
            App.escapeHtml(cat) +
            '" onclick="App.filterWorkflows(&#39;' +
            App.escapeHtml(cat) +
            '&#39;)">' +
            App.escapeHtml(data.categories[cat].label) +
            '</button>';
        }
      }
  ...
  renderWorkflowGrid(workflows) {
    ...
    grid.innerHTML = workflows
      .map(function (wf) {
        ...
        return (
          '<div class="workflow-card" data-category="' +
          App.escapeHtml(wf.category) +
          '" onclick="App.openWorkflowRunner(&#39;' +
          App.escapeHtml(wf.name) +
          '&#39;)" style="--wf-color: ' +
          App.escapeHtml(wf.color) +
          '">' +
          '<div class="workflow-card-icon">' +
          icon +
          '</div>' +
          '<div class="workflow-card-name">' +
          App.escapeHtml(wf.description) +
          '</div>' +
          '<span class="workflow-card-cat">' +
          App.escapeHtml(wf.category) +
          '</span>' +
          '</div>'
        );
  ...
  filterWorkflows(cat) {
    ...
    var filtered = (this.workflowsData || []).filter(function (wf) {
      return wf.category === cat;
    });
    this.renderWorkflowGrid(filtered);
  },

  openWorkflowRunner(name) {
    var wf = (this.workflowsData || []).find(function (w) {
      return w.name === name;
    });
    if (!wf) return;
    ...
    fieldsEl.innerHTML = wf.inputFields
      .map(function (f) {
        var inputHtml;
        if (f.type === 'textarea') {
          inputHtml =
            '<textarea class="tool-input workflow-input" name="' +
          ...
          var opts = (f.options || [])
            ...
            .join('');
          inputHtml =
            '<select class="tool-select workflow-input" name="' + App.escapeHtml(f.name) + '">' + opts + '</select>';
        } else {
          inputHtml =
            '<input type="' +
            (f.type === 'number' ? 'number' : 'text') +
            '" class="tool-input workflow-input" name="' +
            App.escapeHtml(f.name) +
            '" placeholder="' +
            App.escapeHtml(f.placeholder || '') +
            '" value="' +
  ...
  async submitWorkflow(e) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        output.innerHTML =
          '<span class="cmd-error">' + App.escapeHtml(errData.error || 'HTTP ' + res.status) + '</span>';
  ...
  async loadSessions(searchQuery) {
    ...
    try {
      ...
      stats.innerHTML =
        ...
        data.total +
        '</strong> sessions</span>' +
        (data.hasMore ? '<span class="stat-chip">showing latest 50</span>' : '') +
        (data.sessions.length > 0
          ? '<button class="btn-select-mode" onclick="App.toggleSelectMode()">' +
          ...
          .map(function (s) {
            ...
            var checkbox = App.selectMode
              ? '<input type="checkbox" class="card-checkbox" data-id="' +
                App.escapeHtml(s.id) +
                '" onchange="App.updateBatchCount()" onclick="event.stopPropagation()" />'
              : '';
            var deleteBtn = App.selectMode
              ? ''
              : '<button class="card-delete-btn" onclick="event.stopPropagation();App.deleteSession(\'' +
                App.escapeHtml(s.id) +
                '\')" title="Delete session">&times;</button>';

            return (
              '<div class="card session-card' +
              (App.selectMode ? ' selectable' : '') +
              '">' +
              deleteBtn +
              checkbox +
              ...
              App.escapeHtml(App.truncate(title, 60)) +
              '</div>' +
              '<div class="card-meta">' +
              '<span class="card-msg-count">' +
              (s.messageCount || 0) +
              ' msgs</span>' +
              '<span class="card-date" title="' +
              App.escapeHtml(fullDate) +
              '">' +
              App.escapeHtml(date) +
              '</span>' +
              modelBadge +
  ...
  async loadSessionDetail(id) {
    ...
    try {
      ...
      detail.innerHTML =
        ...
        this.escapeHtml(this.truncate(title, 80)) +
        '</div>' +
        ...
        '<button class="btn-continue" onclick="App.resumeSession(\'' +
        this.escapeHtml(id) +
  ...
  async resumeSession(sessionId) {
    try {
      ...
      indicator.innerHTML =
        '<div class="chat-msg-content" style="color:var(--text-muted);font-size:11px;text-align:center">' +
        'Session resumed (' +
        data.messageCount +
        ' messages loaded). Continue the conversation below.' +
        '</div>';
  ...
  async deleteSession(sessionId) {
    ...
    try {
      ...
      this.loadSessions(
        document.getElementById('session-search') ? document.getElementById('session-search').value.trim() : '',
      );
    } catch (err) {
  ...
  onToolSelected(toolName) {
    ...
    if (this.toolsData) {
      for (let i = 0; i < this.toolsData.length; i++) {
        if (this.toolsData[i].name === toolName) {
          tool = this.toolsData[i];
          break;
        }
      }
    }
    ...
    for (let i = 0; i < keys.length; i++) {
      const key = keys[i],
        schema = props[key],
        isReq = required.indexOf(key) >= 0,
        desc = schema.description || '',
        type = schema.type || 'string';
      if (type === 'boolean') {
        html +=
          '<div class="tool-field"><label class="tool-label"><input type="checkbox" name="' +
          this.escapeHtml(key) +
          '" class="tool-checkbox" />' +
          this.escapeHtml(key) +
          (isReq ? ' *' : '') +
          '</label><div class="tool-field-desc">' +
          this.escapeHtml(desc) +
          '</div></div>';
      } else {
        const it = type === 'number' ? 'number' : 'text';
        html +=
          '<div class="tool-field"><label class="tool-label">' +
          this.escapeHtml(key) +
          (isReq ? ' *' : '') +
          '</label><input type="' +
          it +
          '" name="' +
          this.escapeHtml(key) +
          '" class="tool-input" placeholder="' +
          this.escapeHtml(desc) +
          '" /><div class="tool-field-desc">' +
          this.escapeHtml(desc) +
          '</div></div>';
      }
    }
  ...
  async executeSelectedTool() {
    ...
    for (let i = 0; i < keys.length; i++) {
      const key = keys[i],
        schema = props[key],
        input = document.querySelector('#tool-form [name="' + key + '"]');
      if (!input) continue;
    ...
    try {
      ...
      if (data.is_error) {
        ...
        resultEl.innerHTML =
          '<pre class="cmd-success">' +
          this.escapeHtml(data.result) +
          '</pre><div class="cmd-meta">' +
          data.duration_ms +
          'ms</div>';
      }
    } catch (err) {
  ...
  setupCodeAGIListeners() {
    ...
    if (createBtn) {
      createBtn.onclick = function () {
        self.createCodeAGIMission();
      };
      missionInput.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') self.createCodeAGIMission();
      });
    }
    ...
    document.querySelectorAll('.codeagi-tab').forEach(function (tab) {
      tab.onclick = function () {
        document.querySelectorAll('.codeagi-tab').forEach(function (t) {
          t.classList.remove('active');
        });
        tab.classList.add('active');
        document.querySelectorAll('.codeagi-tab-content').forEach(function (c) {
    ...
    document.querySelectorAll('.codeagi-mem-tab').forEach(function (tab) {
      tab.onclick = function () {
        document.querySelectorAll('.codeagi-mem-tab').forEach(function (t) {
          t.classList.remove('active');
        });
        tab.classList.add('active');
  ...
  async createCodeAGIMission() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error('Failed to create mission: HTTP ' + res.status, errData.error || '');
        return;
      }
      var data = await res.json();
      this.loadCodeAGIMissions();
  ...
  async addCodeAGITask() {
    ...
    if (!desc) return;
    var body = {
      mission_id: this.codeagiSelectedMission,
      description: desc,
      action_kind: document.getElementById('codeagi-task-kind').value || undefined,
      path: document.getElementById('codeagi-task-path').value || undefined,
      command: document.getElementById('codeagi-task-cmd').value || undefined,
    };
    try {
      await apiFetch('/api/codeagi/tasks', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      });
  ...
  async loadCodeAGIMissions() {
    ...
    try {
      ...
      if (missions.length === 0) {
        container.innerHTML =
          '<div class="hint-muted" style="padding:1rem;text-align:center">No missions yet. Create one above!</div>';
        return;
      }
      ...
      container.innerHTML = missions
        .map(function (m) {
          var mTasks = tasks.filter(function (t) {
            return t.mission_id === m.id;
          });
          var selected = self.codeagiSelectedMission === m.id ? ' selected' : '';
          return (
            '<div class="codeagi-mission-card' +
            selected +
            '" data-mission-id="' +
            m.id +
            '">' +
            '<div class="codeagi-mission-header"><span class="codeagi-mission-id">' +
            self.escapeHtml(m.id) +
            '</span><span class="codeagi-mission-status ' +
            self.escapeHtml(m.status) +
            '">' +
            self.escapeHtml(m.status) +
            '</span></div>' +
            '<div class="codeagi-mission-desc">' +
            self.escapeHtml(m.description) +
            '</div>' +
            (mTasks.length > 0
              ? '<div class="codeagi-mission-tasks">' +
                mTasks
                  .map(function (t) {
                    return (
                      '<div class="codeagi-task-item"><span class="codeagi-task-dot ' +
                      t.status +
                      '"></span><span>' +
                      self.escapeHtml(t.description) +
                      '</span>' +
                      (t.action_kind ? '<span class="hint-muted">(' + t.action_kind + ')</span>' : '') +
                      '</div>'
                    );
                  })
                  .join('') +
      ...
      container.querySelectorAll('.codeagi-mission-card').forEach(function (card) {
        card.onclick = function () {
          ...
          container.querySelectorAll('.codeagi-mission-card').forEach(function (c) {
      ...
      container.innerHTML =
        '<div class="hint-muted" style="padding:1rem">CodeAGI not available: ' + err.message + '</div>';
    }
  },
  ...
  async runCodeAGICycle() {
    ...
    runBtn.disabled = true;
    runBtn.innerHTML =
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="10" y1="15" x2="10" y2="9"/><line x1="14" y1="15" x2="14" y2="9"/></svg> Running...';
    outputSection.style.display = 'block';
    stepsEl.innerHTML = '';
    var self = this;
    try {
      var res = await apiFetch('/api/codeagi/run', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ max_cycles: 1 }),
      });
      var reader = res.body.getReader();
      var decoder = new TextDecoder();
      var buffer = '';
      while (true) {
        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          if (payload === '[DONE]') break;
          try {
            self.appendCodeAGIStep(stepsEl, JSON.parse(payload));
          } catch (e) {}
        }
      }
    } catch (err) {
      self.appendCodeAGIStep(stepsEl, { type: 'error', text: err.message });
    } finally {
      runBtn.disabled = false;
      runBtn.innerHTML =
        '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"/></svg> Run Cycle';
      self.loadCodeAGIMissions();
      self.loadCodeAGIWorkspace();
    }
  },
  ...
  appendCodeAGIStep(container, ev) {
    ...
    if (ev.type === 'cycle_data' && ev.data) {
      ...
      if (d.plan) {
        ...
        s.className = 'codeagi-step plan';
        s.innerHTML =
          '<div class="codeagi-step-label">Plan</div><div>' +
          this.escapeHtml(JSON.stringify(d.plan.steps || d.plan, null, 1).substring(0, 300)) +
          '</div>';
        container.appendChild(s);
      }
      if (d.verification) {
        var s = document.createElement('div');
        s.className = 'codeagi-step verify';
        s.innerHTML =
          '<div class="codeagi-step-label">Verify</div><div>' +
          this.escapeHtml(JSON.stringify(d.verification).substring(0, 200)) +
          '</div>';
        container.appendChild(s);
      }
      if (d.critique) {
        var s = document.createElement('div');
        s.className = 'codeagi-step critique';
        s.innerHTML =
          '<div class="codeagi-step-label">Critique</div><div>' +
          this.escapeHtml(JSON.stringify(d.critique).substring(0, 200)) +
          '</div>';
        container.appendChild(s);
      }
      if (d.action_outcome) {
        var s = document.createElement('div');
        s.className = 'codeagi-step execute';
        s.innerHTML =
          '<div class="codeagi-step-label">Execute</div><div>' +
          this.escapeHtml(d.action_outcome.summary || JSON.stringify(d.action_outcome).substring(0, 200)) +
          '</div>';
        container.appendChild(s);
      }
      if (d.reflection) {
        var s = document.createElement('div');
        s.className = 'codeagi-step reflect';
        s.innerHTML =
          '<div class="codeagi-step-label">Reflect</div><div>' +
          this.escapeHtml(d.reflection.summary || JSON.stringify(d.reflection).substring(0, 200)) +
          '</div>';
        container.appendChild(s);
      }
      if (d.status === 'idle' || (d.cycle_trace && d.cycle_trace.stop_reason)) {
        var s = document.createElement('div');
        s.className = 'codeagi-step complete';
        s.innerHTML =
          '<div class="codeagi-step-label">Done</div><div>' +
          this.escapeHtml(d.cycle_trace ? d.cycle_trace.stop_reason : d.status) +
          '</div>';
        container.appendChild(s);
      }
      container.scrollTop = container.scrollHeight;
    ...
    if (ev.type === 'text' || ev.type === 'log') {
      step.className = 'codeagi-step';
      step.textContent = ev.text || '';
    } else if (ev.type === 'status') {
      step.className = 'codeagi-step';
      step.innerHTML = '<div class="codeagi-step-label">Status</div><div>' + this.escapeHtml(ev.text || '') + '</div>';
    } else if (ev.type === 'complete') {
      step.className = 'codeagi-step complete';
      step.innerHTML =
        '<div class="codeagi-step-label">Complete</div><div>' + this.escapeHtml(ev.text || '') + '</div>';
    } else if (ev.type === 'error') {
      step.className = 'codeagi-step error';
      step.innerHTML = '<div class="codeagi-step-label">Error</div><div>' + this.escapeHtml(ev.text || '') + '</div>';
    } else {
      step.className = 'codeagi-step';
      step.textContent = JSON.stringify(ev);
    }
  ...
  async runCodeAGIStream() {
    ...
    streamBtn.innerHTML =
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="10" y1="15" x2="10" y2="9"/><line x1="14" y1="15" x2="14" y2="9"/></svg> Streaming...';
    ...
    try {
      ...
      streamBtn.innerHTML =
        '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg> Stream Cycle';
      self.loadCodeAGIMissions();
      self.loadCodeAGIWorkspace();
    }
  },
  ...
  async loadCodeAGIWorkspace(subdir) {
    ...
    try {
      ...
      var self = this;
      if (files.length === 0) {
        filesEl.innerHTML = '<div class="hint-muted" style="padding:0.5rem">Empty</div>';
        return;
      }
      ...
      if (p) {
        ...
        entries +=
          '<div class="codeagi-file-item" data-action="dir" data-path="' +
          parent +
          '"><span class="codeagi-file-icon">&#11014;</span><span>..</span></div>';
      }
      entries += files
        .map(function (f) {
          ...
          return (
            '<div class="codeagi-file-item" data-action="' +
            (f.type === 'directory' ? 'dir' : 'file') +
            '" data-path="' +
            self.escapeHtml(fullPath) +
            '"><span class="codeagi-file-icon">' +
            icon +
            '</span><span>' +
            self.escapeHtml(f.name) +
            '</span>' +
            (size ? '<span class="codeagi-file-size">' + size + '</span>' : '') +
            '</div>'
          );
        })
        .join('');
      ...
      filesEl.querySelectorAll('.codeagi-file-item').forEach(function (item) {
        item.onclick = function () {
          item.dataset.action === 'dir'
            ? self.loadCodeAGIWorkspace(item.dataset.path)
            : self.previewCodeAGIFile(item.dataset.path);
        };
      });
    } catch (err) {
      filesEl.innerHTML = '<div class="hint-muted">Workspace not available</div>';
    }
  },
  ...
  async loadCodeAGIMemory(type) {
    ...
    try {
      ...
      if (!Array.isArray(items) || items.length === 0) {
        container.innerHTML =
          '<div class="hint-muted" style="padding:1rem;text-align:center">No ' + type + ' yet</div>';
        return;
      }
      ...
      if (type === 'reflections') {
        container.innerHTML = items
          .map(function (r) {
            return (
              '<div class="codeagi-memory-item"><div class="codeagi-memory-item-title">' +
              App.escapeHtml(r.summary || r.next_action || 'Reflection') +
              '</div>' +
              (r.lessons && r.lessons.length
                ? '<div class="codeagi-memory-item-body">Lessons: ' + App.escapeHtml(r.lessons.join(', ')) + '</div>'
                : '') +
              '<div class="codeagi-memory-item-meta">' +
              App.escapeHtml(r.mission_id || '') +
              ' \u00B7 ' +
              App.escapeHtml(r.created_at || '') +
              '</div></div>'
            );
          })
          .join('');
        ...
        container.innerHTML = items
          .map(function (f) {
            return (
              '<div class="codeagi-memory-item"><div class="codeagi-memory-item-title">' +
              App.escapeHtml(f.content || f.fact || JSON.stringify(f).substring(0, 100)) +
              '</div>' +
              (f.tags
                ? '<div class="codeagi-memory-item-body">Tags: ' +
                  App.escapeHtml(Array.isArray(f.tags) ? f.tags.join(', ') : String(f.tags)) +
                  '</div>'
                : '') +
              '<div class="codeagi-memory-item-meta">Confidence: ' +
              App.escapeHtml(String(f.confidence || '?')) +
              '</div></div>'
            );
          })
          .join('');
        ...
        container.innerHTML = items
          .map(function (p) {
            return (
              '<div class="codeagi-memory-item"><div class="codeagi-memory-item-title">' +
              App.escapeHtml(p.title || 'Procedure') +
              '</div><div class="codeagi-memory-item-body">Trigger: ' +
              App.escapeHtml(p.trigger || '?') +
              '</div>' +
              (p.steps
                ? '<div class="codeagi-memory-item-body">' + App.escapeHtml(p.steps.join(' \u2192 ')) + '</div>'
                : '') +
              '<div class="codeagi-memory-item-meta">Uses: ' +
              (p.use_count || 0) +
              '</div></div>'
            );
          })
      ...
      container.innerHTML = '<div class="hint-muted">Failed to load: ' + err.message + '</div>';
    }
  },
  ...
  async loadCodeAGITraces() {
    ...
    try {
      ...
      if (!Array.isArray(traces) || traces.length === 0) {
        container.innerHTML = '<div class="hint-muted" style="padding:1rem;text-align:center">No traces yet</div>';
        return;
      }
      ...
      container.innerHTML = traces
        .map(function (t) {
          ...
          return (
            '<div class="codeagi-trace-card"><div class="codeagi-trace-header"><span>' +
            App.escapeHtml(t.mission_id || '?') +
            '</span><span class="hint-muted">' +
            steps +
            ' steps \u00B7 ' +
            App.escapeHtml(t.stop_reason || '?') +
            '</span></div>' +
            (t.steps
              ? '<div class="codeagi-trace-steps">' +
                t.steps
                  .map(function (s) {
                    var c = s.action_outcome && s.action_outcome.ok ? '#00ff41' : '#ff3c3c';
                    return '<div class="codeagi-trace-step-dot" style="background:' + c + '"></div>';
                  })
                  .join('') +
                '</div>'
              : '') +
      ...
      container.innerHTML = '<div class="hint-muted">Failed to load traces</div>';
    }
  },
...
function updateStreamingStatus(msg) {
  ...
  if (statusEl) {
    ...
    el.style.cssText =
      'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:rgba(0,212,255,0.1);border:1px solid rgba(0,212,255,0.3);color:#00d4ff;padding:6px 16px;border-radius:20px;font-size:13px;z-index:1000;transition:opacity 0.3s;';
    el.textContent = msg;
