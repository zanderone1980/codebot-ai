...
10. **PR 10+ — remaining Phase 1 connectors**, one per PR (Google Calendar, filesystem, browser tooling). Each must pass `assertContractClean` cleanly.
...
| **Connector contract compliance** | **WIRED** | `src/connectors/contract-compliance.test.ts` (2 tests). Runs on every CI matrix entry. Current per-connector readout: `github 7/7 (100%)`, `gmail 5/5 (100%)`, every other connector `0/X (0%)`. Aggregate: **12/50 (24%)**. New connector PRs must call `assertContractClean(new MyConnector())` and pass with zero violations (hard-fail). Existing connectors report-only. |
...
*Last updated 2026-04-26 (PR 9 landed: GitHub migrated to the §8 contract — 7 existing actions, no new ones. Both mutating verbs (`create_issue`, `create_pr`) declare `idempotency: { kind: 'unsupported', reason: ... }` because GitHub REST does not provide a client-supplied idempotency key. The `create_pr` reason explicitly distinguishes (head, base) 422-rejection from idempotency — the connector does NOT treat that rejection as safe-retry semantics. New `isGithubAuthError` classifier: 401 always reauth; 403 only on auth-class messages (bad credentials, scope, SSO); 403 + rate-limit / abuse-detection explicitly does NOT trigger reauth. Compliance: 5/50 → 12/50 (24%); github 7/7. §12 connector-compliance row updated.). Earlier: §12 honesty pass; honesty pass PR #33; PR 8 Gmail migration; PR 7 + refinement (connector contract); PR 6 budget; PR 5 router; PR 4 capability gating; PR 3 label population; PR 2 capability slot; pre-sprint security work. Against `main @ 5abd1ad` + this PR.*
