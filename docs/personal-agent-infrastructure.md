...
|---|---|---|
| **Audit-chain integrity** | **WIRED** | `src/audit.test.ts` (16 tests) runs on every CI matrix entry. Covers tamper detection on legacy actions + post-PR-5/6 actions (`router:switch`, `budget_block`), multi-session interleaving, 100-entry bulk chains. A weakened verifier or a broken chain in any test session fails the build. Production audit log spot-checked 2026-04-25: 1,074 entries / 283 sessions / 0 invalid. |
...
| **Connector contract compliance** | **WIRED** | `src/connectors/contract-compliance.test.ts` (2 tests). Runs on every CI matrix entry. Current per-connector readout: `gmail 5/5 (100%)`, every other connector `0/X (0%)`. Aggregate: **5/50 (10%)**. New connector PRs must call `assertContractClean(new MyConnector())` and pass with zero violations (hard-fail). Existing connectors report-only. |
...
*Last updated 2026-04-26 (§12 honesty pass — every measurement row carries an explicit status tag: WIRED / PARTIAL / NOT IMPLEMENTED / N/A. Banned aspirational phrases ("will," "should," "tracked from PR X onward," "drives upward," "goal: reach") in measurement contexts. Real status: 3 WIRED rows (audit-chain integrity, capability-label coverage, connector contract compliance), 3 PARTIAL rows (tool-calls-without-audit by code-shape only, model-router cost is per-session-only with no trend rollup, denied-action-rate has data but no rate computation), 1 NOT IMPLEMENTED (approval latency), 2 N/A (time-to-add-a-connector wall-clock, cross-device audit until cross-device exists). PR 7 history entry also corrected — was "drives upward as connectors migrate," now states the actual sequence). Earlier: 2026-04-25 honesty pass PR #33 (test audit pollution + audit-chain CI tests); PR 8 Gmail migration; PR 7 + refinement (connector contract); PR 6 budget; PR 5 router; PR 4 capability gating; PR 3 label population; PR 2 capability slot; pre-sprint security work. Against `main @ 367817a` + this PR.*
