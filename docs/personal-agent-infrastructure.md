...
*Last updated 2026-04-25 (PR 4 landed: `_prepareToolCall` escalates permission via `escalatePermissionFromCapabilityLabels`; `ToolRegistry.register` rejects `move-money`; capability-driven `always-ask` is immune to `autoApprove` — §7 note added clarifying the partial-invariant scope vs. legacy static `'always-ask'`; §10 PR 4 marked done). Earlier passes: 2026-04-25 PR 3 (label population + CI coverage enforcement); 2026-04-25 PR 2 (CapabilityLabel union + slot); 2026-04-25 split `move-money` from `spend-money` and reframed Phase 2; 2026-04-24 engineering-contract discipline; 2026-04-24 user-owned multi-device-over-time vision. Against `main @ 472d118` + this PR.*
