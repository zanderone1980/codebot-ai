...
| **Idempotency / duplicate-submit protection** | Where the underlying service supports it (Gmail message-id, GitHub PR number, calendar event-id), the connector declares `idempotencyKeyArg` naming the args field that carries the user-supplied key, and the service rejects a second call with the same key as a no-op. Where the service does NOT support it, the connector declares `idempotencyUnsupportedReason` (a non-empty short technical sentence) — that's the explicit "documented gap" the contract requires. Setting NEITHER on a mutating verb fails `assertContractClean`. Setting both is allowed (a connector might support partial dedup but want to record a known limitation). |
...
7. **PR 7 — connector contract foundation.** ✓ Landed in PR #30. Adds 5 optional fields to `ConnectorAction` (`capabilities`, `preview`, `idempotencyKeyArg`, `idempotencyUnsupportedReason`, `redactArgsForAudit`) and 1 to `Connector` (`vaultKeyName`). Exports `ConnectorReauthError` (catchable by `kind === 'reauth-required'` before any string formatting). Validator + score helpers in `src/connectors/connector-contract.ts`. Test-only fixture `TestConnector` proves the contract scaffold end-to-end. Existing 11 production connectors compile unchanged; the compliance test reports their score (starts at 0/50 actions clean, drives upward as connectors migrate). **No external behavior change** — AppConnectorTool's tool-level capability union is unchanged; per-verb gating is a future PR. AppConnectorTool now catches `ConnectorReauthError` and returns a recognizable structured error string. Idempotency declaration is required for mutating verbs, with an honest "service doesn't support it" escape hatch via `idempotencyUnsupportedReason` so the contract never forces fake dedup args.

8. **PR 8+ — connectors, one PR per Phase 1 connector**, in the order listed in §8. Each PR migrates an existing connector or lands a new one and must pass `assertContractClean` cleanly.
...
| **Connector contract compliance** | `src/connectors/contract-compliance.test.ts` prints per-connector score on every CI run. Aggregate ratio = `compliantActions / totalActions` across all 11 production connectors. | **Baseline at PR 7: 0/50 (0%) — every existing connector predates the contract.** Drives upward as PRs migrate connectors. New connector PRs must pass `assertContractClean` (hard-fail). Goal: reach 100% before declaring §8 fully shipped. |
...
*Last updated 2026-04-25 (PR 7 landed: connector contract foundation — 4 optional fields on `ConnectorAction` + 1 on `Connector`, `ConnectorReauthError` structural class, validator + score helpers, test-only fixture proving the contract; existing 11 connectors compile unchanged; compliance baseline 0/50 actions clean — drives upward as connectors migrate; AppConnectorTool catches `ConnectorReauthError`; §8 note added documenting the existing-vs-new hard-fail split; §10 PR 7 marked done; §12 adds compliance-score measurement). Earlier passes: 2026-04-25 PR 6 (per-session budget cap + threshold audits); 2026-04-25 PR 5 (router rename + wire-up, same-provider only); 2026-04-25 PR 4 (capability-driven gating + move-money rejection); 2026-04-25 PR 3 (label population + CI enforcement); 2026-04-25 PR 2 (CapabilityLabel union + slot); 2026-04-25 split `move-money` from `spend-money`; 2026-04-24 engineering-contract discipline; 2026-04-24 user-owned multi-device-over-time vision. Against `main @ 7a338f3` + this PR.*
