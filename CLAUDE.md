...
Active branch: `main` (linear history, conventional tag prefixes: [FIX] [INFRA] [REFACTOR] [DOCS] [SECURITY] [TEST] [LEARN] [EVAL] [SOLVE])
...
- Commit prefixes (enforced by `.agent-guardrails.json`): `[FIX]` `[INFRA]` `[REFACTOR]` `[DOCS]` `[SECURITY]` `[TEST]` `[LEARN]` `[EVAL]` `[SOLVE]`. Do NOT introduce `[FEAT]` — the guardrail rejects it; "new infrastructure" is `[INFRA]`, "user-facing fix" is `[FIX]`.
