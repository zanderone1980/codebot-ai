...
19. **PR 21+ — dashboard always-ask permission UX + remaining Phase 1 connectors** (Jira, Linear, then Replicate last because paid generation needs the spend-money pass). PR 21 surfaces a visible Approve/Deny card in the Electron renderer when an always-ask tool fires, consuming the `ConnectorPreview` shape the §8-migrated connectors already return.
