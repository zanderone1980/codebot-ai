...
9. **PR 9+ — remaining Phase 1 connectors**, one per PR (GitHub, Google Calendar, filesystem, browser tooling). Each must pass `assertContractClean` cleanly.
...
| Signal | Source | Bar |
|---|---|---|
| **Audit-chain integrity** | `src/audit.test.ts` exercises `AuditLogger.verify` on every CI run: full action union (14 actions per chain), tampering detection (router:switch and budget_block specifically), multi-session interleaving, 100-entry bulk chain. **Honesty-pass note (2026-04-25):** earlier wording promised CI would read `~/.codebot/audit/` from the host, which doesn't make sense — CI runners have no production audit data. Real enforcement is the test suite, which DOES run on every CI matrix entry and DOES catch tampering. Production audit log was independently verified at PR-7-time: 1,074 entries / 283 sessions / 0 invalid chains. | Must pass on every CI run. A broken chain or weakened verifier fails the build. |
| **Tool calls without an audit entry** | Honest scope: NOT enforced by an automated counter today. The path is single (`agent/tool-executor.ts:executeSingleTool` is the one place tools run, and it always writes an audit entry on the success or error branch). Reviewers reject any new tool-execute path that bypasses it. **Wiring an explicit counter is a deferred enhancement** — when the agent loop ever grows a second exec path, this needs upgrading from "single path discipline" to a runtime invariant. | Currently disciplined by code shape, not by CI. Should reach zero by construction; audit if a second exec path ever lands. |
...
| **Model-router cost per session** | Token-tracker rollup, written to a session summary. **Visible on every CLI session as of PR 6** (banner shows `Cost` + `Budget`). | Tracked from PR 5 onward. Cheap-first heuristic should drive the median session below the pre-router single-model baseline. |
...
| **Connector contract compliance** | `src/connectors/contract-compliance.test.ts` prints per-connector score on every CI run. Aggregate ratio = `compliantActions / totalActions` across all 11 production connectors. | Baseline at PR 7: 0/50 (0%). **Current as of PR 8: 5/50 (10%)** — gmail migrated to 5/5. Drives upward as further PRs migrate connectors. New connector PRs must pass `assertContractClean` (hard-fail). Goal: reach 100% before declaring §8 fully shipped. |
...
*Last updated 2026-04-25 (Honesty-pass PR: end-to-end verification of all behavior-changing layers — router fires on real Agent w/ enabled config, budget thresholds + block fire as cost climbs, PR 4 capability-label deny path emits new reason string, Gmail preview is pure-from-args. Found two real bugs: (1) tests were polluting `~/.codebot/audit/` because Agent constructed AuditLogger without a dir override — fixed by adding `auditDir?` opt + sweeping 7 test files via `makeTestAuditDir()`; (2) §12 audit-chain row was half-aspiration — rewrote to match reality with 5 new tests in `audit.test.ts` exercising the full action union including PR 5/6 additions. Production audit log verified: 1,074 entries / 283 sessions / 0 invalid chains.). Earlier: PR 8 Gmail migration; PR 7 + refinement (connector contract); PR 6 budget; PR 5 router; PR 4 capability gating; PR 3 label population; PR 2 capability slot; pre-sprint security work. Against `main @ 1c1ba2b` + this PR.*
