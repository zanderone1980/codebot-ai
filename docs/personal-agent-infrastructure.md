...
*Last updated 2026-04-25 (PR 5 landed: `src/router.ts` renamed to `fast/strong/reasoning` matching §6 canonical names; wired into agent loop via `Agent.maybeRouteModel()`; `SavedConfig.router?: RouterConfig` added; **same-provider routing only** — cross-provider falls open to current model and audits `router:fallback`; audit actions `switch` / `fallback` added; §6 note explains the PR 5 scope; §10 PR 5 marked done). Earlier passes: 2026-04-25 PR 4 (capability-driven gating + move-money rejection); 2026-04-25 PR 3 (label population + CI enforcement); 2026-04-25 PR 2 (CapabilityLabel union + slot); 2026-04-25 split `move-money` from `spend-money`; 2026-04-24 engineering-contract discipline; 2026-04-24 user-owned multi-device-over-time vision. Against `main @ 1e388ef` + this PR.*
